...
- The served rendering of the manifest global changed from `window.__XHE_BOOT__ =` to `globalThis["__XHE_BOOT__"] =`; no committed snapshot expectation carries that text, so none needed re-recording.
