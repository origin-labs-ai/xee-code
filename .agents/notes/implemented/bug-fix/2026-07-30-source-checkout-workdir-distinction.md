...
The section identifies the path as the “Xee Harness Enhanced implementation checkout.” It says that the checkout location and current working directory are separate values that may differ, forbids inferring the working directory from the checkout path, directs the model to use `pwd`, and limits the checkout's purpose to inspecting or extending XHE itself.
...
The `xhe-app-boot` unit test pins the exact text and its ordering. The CLI keyless PTY smoke inspects the assembled request header. The TUI `source-checkout-workdir` snapshot mounts the section with `/opt/xhe-source`, asks “what's the workdir?” through a recorded DeepSeek V4 turn, and requires the replayed transcript to run `pwd` and report the generated workspace rather than the checkout.
...
**Remove the source path from the prompt.** Rejected because self-referential XHE tools still need a reliable checkout location when the launcher starts from an unrelated project.
