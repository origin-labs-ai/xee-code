...
The fixed user-global `$CF_HOME/AGENTS.md` has no local overlay and stays base-only.
...
**Extend the overlay to the user-global scope.** Deferred: `$CF_HOME` is a single fixed `AGENTS.md` with no committed base to supplement, so it stays base-only until a concrete need appears.
