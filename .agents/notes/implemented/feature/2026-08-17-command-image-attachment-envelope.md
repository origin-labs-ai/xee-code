...
- The commands package now depends on `xhe-attachment` and `xhe-llm`, and `commands/execute` carries a required `images` wire parameter — every caller states its envelope explicitly.
