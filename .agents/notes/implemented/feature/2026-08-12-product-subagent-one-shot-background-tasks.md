...
| Foreground or background choice | `xhe-tool-subagent` | Resolve `run_in_background` under `one-shot` policy | Omission is foreground; explicit `true` returns a Job id |
| Job id, state, output, cancellation, and notice | `ctx.jobs` and `xhe-tool-jobs` | Register and present the existing one-shot run | Generic job tools collect or stop the run for the exact parent |
| Native result, optional diagnostic, and process quiescence | Product provider and `xhe-subprocess` | Produce one final result and release one process tree | Job settlement and foreground return consume the same result and both wait for disposal |
...
A standalone custom composition that enables one-shot background execution must provide the product provider plus the complete generic Job capability: `xhe-jobs-local` as the Job provider and `xhe-tool-jobs` as the model-facing consumer. A Profile based on `xhe-base` already has the Job capability and adds only the optional product provider before enabling the preset tool row. A product tool without the Job runtime can still execute in the foreground, but an explicit background request fails the existing Job preflight instead of publishing an uncollectable id.
