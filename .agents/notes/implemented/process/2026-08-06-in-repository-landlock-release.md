...
- **Give the launcher the root Xee Harness Enhanced version and publish the complete monorepo recursively** — rejected because this change owns one three-package public family, not the independent `@origin-ai/xhe-*` baseline. The [artifact-first npm baseline proposal](../../proposed/process/2026-08-04-artifact-first-npm-baseline-publication.md) explicitly keeps native workspaces outside its target set.
