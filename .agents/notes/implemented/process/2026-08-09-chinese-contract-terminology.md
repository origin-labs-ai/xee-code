# Agent Note: Standardize Chinese contract terminology on 

Status: implemented

## Problem

The Chinese documentation rendered English `contract` inconsistently as `` and ``, sometimes within one file or paragraph. The terminology table prescribed ``, while reviewed incremental proofreading selected the more natural engineering rendering ``. Leaving the table and corpus split made either choice fail the repository's terminology rule and allowed later translations to reintroduce the disagreement.

English `convention` also commonly renders as ``. That overlap is intentional: ordinary Chinese engineering prose uses `` for both concepts, and context normally carries whether a statement is descriptive practice or a binding interface rule. Where an English sentence explicitly contrasts a convention with a contract, the Chinese sentence must preserve the distinction through wording such as `` versus ``, rather than mechanically giving every `convention` the same rendering.

## Decision

The terminology source of truth defines `contract` as `` and `adapter contract` as `（adapter contract）` on first mention. Every active Chinese documentation pair follows that ruling; archived Agent Notes remain frozen. Unpaired bilingual calibration assets and the translation prompt's explanatory prose follow the same terms so they cannot teach the superseded rendering.
...
**Keep `contract` as ``.** Rejected because the reviewed corpus consistently preferred `` for technical interfaces, lifecycle guarantees, and behavioral boundaries, and maintaining the older term would require reverting accepted proofreading across many documents.
...
**Permit both `` and `` for `contract`.** Rejected because it preserves the exact inconsistency that made package families and even individual paragraphs disagree.
