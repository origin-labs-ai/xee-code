...
- **Six merge-extensible maps** (~370 LOC of core types): `ContentBlockMap`, `MessageSourceMap`, `FinishReasonMap` (in `xhe-llm`); `TurnTriggerMap`, `TurnEndReasonMap`, `SessionEventMap` (in `xhe-session`).
- **~10 `declare module` augmentation sites** across `xhe-agent`, `xhe-agent-loop`, `xhe-shell`, `xhe-llm`, `xhe-session`, `xhe-session-persistence`, `xhe-system-prompt`, `xhe-tools` — each would move from declaration merging to a runtime `register()` call.
- **The event producers** — 16 `session.append(...)` call sites in the loop — unchanged in shape but now validated at the boundary.
- **~7 switch-consumers** that branch on these unions: `deriveMessages` and the package-owned invariant companion (`xhe-session`), `BlockAssembler` (`xhe-llm`), both LLM adapters (`xhe-llm-deepseek`, `xhe-llm-pi-ai`), and the tool schema layer (`xhe-tools`). The `assertNever`-on-closed-unions vs fall-through-on-extensible-unions convention (a documented lint rule) would need rethinking — runtime variants are not statically exhaustive.
- **The `defineTool` `InferArgs` DSL** (`xhe-tools`), which derives zero-cast `execute` arg types from a compile-time schema spec — the showcase of the current approach.
