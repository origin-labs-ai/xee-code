...
- `examples/echo-agent` moved to the one-shot `xhe-cli-demo` app; `xhe-cli-demo` gained `-p/--prompt` as the flag form of the single task (mutually exclusive with the positional).
...
- The SDK wizard's `stdio` run interface became `tui` (`RunInterface = 'acp' | 'tui' | 'embed'`), contributing a `xhe-tui` entry instead of `xhe-stdio`; the generated `index.ts` guards TTY before `startSDK` for the same pre-boot fail-loud reason as the tui-demo bin.
...
- **Keep `xhe-stdio` as a pipe/automation channel without the repl demo** — rejected: its automation role duplicated `xhe-cli-demo` with a weaker contract (unstructured transcript, EOF-exit heuristics vs. one durable turn ending and format-pure output).
