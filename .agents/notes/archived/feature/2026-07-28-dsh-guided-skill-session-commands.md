...
Two recurring flows begin with the user manually invoking one skill and answering its questions: migrating from another coding agent, and upgrading this checkout. Both require the user to know the skill exists and to type `/skill:xhe-migrate` or `/skill:xhe-upgrade` as the session's first turn. A dedicated entry command that drops the user straight into that guided session removes the discovery step.

## Decision

`dsh migrate` and `dsh upgrade` boot the ordinary TUI as a fresh session whose first turn auto-invokes a bundled skill (`xhe-migrate`, `xhe-upgrade`), exactly as if the user typed `/skill:<name>` and pressed Enter.
...
No keyless PTY snapshot: per the maintainer's scope call for this change, unit coverage plus interactive verification suffices, and the seed rides the already-snapshotted `/skill:` render path. Both commands were verified interactively in tmux from a scratch cwd: `dsh migrate` loaded `xhe-migrate` and asked which source agent; `dsh upgrade` loaded `xhe-upgrade`, which pulled in `xhe-customize` and began checkout discovery.
...
**Seed a natural-language instruction ("use the xhe-migrate skill…") instead of `/skill:<name>`.** Rejected here: the literal skill-invocation path renders the skill body into the first turn deterministically, identical to the manual command, rather than depending on the model choosing to load the skill.
