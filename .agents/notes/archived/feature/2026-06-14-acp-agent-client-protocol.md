...
The current protocol contract lives in the [`xhe-acp` package README](../../../../packages/acp/acp/README.md).
...
**Inject the concrete `agentLoop`** — rejected. Agent creation, resume, idle observation, and disposal are interface-level ownership operations on `xhe-agent`; a UI plugin does not need a dependency-rule exception.
