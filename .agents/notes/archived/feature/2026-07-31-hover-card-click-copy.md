...
**Change the common Chinese `copied` label from `` to ``.** That would alter every existing copy control to satisfy one card interaction. The Workspace dictionary owns the card-specific wording instead.
