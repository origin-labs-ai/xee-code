...
Add `FileSystem.listDir(target, signal?)` to `@origin-ai/xhe-fs`.
...
**Keep directory enumeration in each consumer.** Rejected. That would bind product packages such as `xhe-skill` to Node/local filesystem behavior and bypass policy/remote/sandboxed backends.
