# Agent Note: Fold the persistence interface into xhe-session

Status: rejected — the separate persistence Service Definition package is the intended modular role split for the durable-persistence capability seam. Folding it into `xhe-session` would reduce package count at the cost of a cleaner backend boundary.

## Problem

`xhe-session-persistence` is a Service Definition package whose main concepts are already owned by `xhe-session`: `SessionHeader`, `SessionEvent`, `SessionId`, `session/event`, and `session/flush`. The package adds the abstract `SessionPersistence` service, the shared write coordinator, and contract helpers. Provider packages depend on it, and `agent-loop` has to optionally find a sibling service for resume.
...
- `@origin-ai/xhe-session-persistence` is removed as a package.
- `xhe-session` exports the persistence service type, coordinator, and contract helpers.
- JSONL and SQLite backend packages depend on `xhe-session` directly.
