...
Compaction is split between `@origin-ai/xhe-compaction`, which owns an abstract two-method service and shared types, and `@origin-ai/xhe-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `@origin-ai/xhe-compaction` and remove `@origin-ai/xhe-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@origin-ai/xhe-compaction-basic` and its workspace/package metadata are removed.
- `@origin-ai/xhe-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
