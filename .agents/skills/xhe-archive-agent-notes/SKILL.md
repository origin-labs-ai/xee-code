...
Run the archive verifier's focused test, `pnpm run verify-archived-agent-notes`, `pnpm run doc-sync`, `pnpm run lint`, and `git diff --check`; select any additional evidence through [xhe-pre-push-checks](../xhe-pre-push-checks/SKILL.md).
