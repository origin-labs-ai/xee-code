# DSH Popular Skills - Slash Commands Registry

This document defines all slash commands for the `dsh-popular-skills` collection.
Commands are registered via `ctx.commands.register()` in DSH's interaction system.
