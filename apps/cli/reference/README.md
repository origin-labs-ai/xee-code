...
Bundle names resolve from the dsh installation first, then from the profile directory. In-box bundles (`@origin-ai/cf-base`, `@origin-ai/cf-web-app`, `@origin-ai/cf-headless`) therefore always come from the same installation as the running `dsh`; out-of-tree bundles come from the profile's pnpm-managed `node_modules`. A bare plugin `name` in any patch row resolves through the profile directory's Node parent-walk, which reaches the maintained installation fallback `$XHE_HOME/profiles/node_modules` (one symlink per package the installation's app and bundles depend on, healed on every launch).
...
dsh plugin --profile <name> add @origin-ai/cf-subagent-claude-code
dsh plugin --profile <name> add @origin-ai/cf-subagent-codex @origin-ai/cf-subagent-claude-code
dsh plugin --profile <name> remove @origin-ai/cf-subagent-codex
dsh plugin --profile <name> remove @origin-ai/cf-subagent-claude-code
```
...
Install external plugin bundles through `dsh plugin --profile <name> add <package-or-git-spec>`. The installed package owns its dependencies and contributes its declared `cordis.patch.yml` layer. The CLI also ships `@origin-ai/cf-mcp-client` as a dependency for patch layers, but no MCP server is enabled by default because each server command is trusted executable code outside the agent sandbox.
