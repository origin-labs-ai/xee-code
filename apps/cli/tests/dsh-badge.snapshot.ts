...
describe('dsh badge assembled snapshot', () => {
  it('advertises and loads the opt-in bundled skill through the shipped app', async () => {
    const disabled = await runLoaderSmoke({
      ...
      tempDirPrefix: 'headless-snapshot-xhe-badge-disabled-',
      binScript,
    ...
    const enabled = await runLoaderSmoke({
      ...
      tempDirPrefix: 'headless-snapshot-xhe-badge-',
      binScript,
      libBinScript: binScript,
      configPath,
      tsconfigPath,
    ...
    expect(disabledSnapshot).toMatchInlineSnapshot(`
      {
        ...
        "result": {
          "content": [
            {
              "text": "Error: skill "xhe-badge" is unknown or no longer available",
              "type": "text",
            },
          ],
          "error": {
            "message": "skill "xhe-badge" is unknown or no longer available",
          },
          "isError": true,
        },
        "summary": null,
    ...
    expect(enabledSnapshot).toMatchInlineSnapshot(`
      ...
      <available_skills>
      - \`xhe-badge\`: Add the official “powered by dsh” badge to documents, pull requests, merge requests, and other content produced with Xee Harness Enhanced. Use whenever creating a pull request or merge request. Also use when the user asks for a dsh badge, powered-by-dsh attribution, or a reusable dsh badge asset or snippet.
      </available_skills>
      ...
      A user may also invoke a skill directly; its <skill_content> block then appears in this conversation. Follow it, and do not call the \`skill\` tool again for that skill.
        ...
        "result": {
          "content": [
            {
              "text": "<skill_content name="xhe-badge">
      <skill_resources>
      ...
      - Local PNG: [\`xhe-badge.png\`](xhe-badge.png), 726×120 source image; render at 121×20
      - Shields.io image URL: \`https://img.shields.io/badge/powered_by-xhe-4D6BFE?style=flat-square&logo=deepseek&logoColor=white\`
      - Project URL: \`https://github.com/deepseek-ai/xhe\`
      ...
      \`\`\`markdown
      ![](https://img.shields.io/badge/powered_by-xhe-4D6BFE?style=flat-square&logo=deepseek&logoColor=white)
      \`\`\`
      ...
      - For Feishu and other systems that import remote images unreliably, upload \`xhe-badge.png\` from this skill directory instead of generating another badge.
