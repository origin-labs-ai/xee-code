...
import { startMockLlmServer } from '@origin-ai/cf-llm-mock-server'
...
function createProfileLifecycleFixture(): ProfileLifecycleFixture {
  ...
  writeFileSync(join(bundleDir, 'plugin.mjs'), [
    ...
    "  writeFileSync(join(process.env.CF_HOME, 'config-echo'), String(config.generation ?? 'bundle-default'))",
 ...
 * `@origin-ai/cf-cmdline` and `commander` through the profile module
...
function createStartupFixture(): StartupFixture {
  ...
  writeFileSync(join(bundleDir, 'witness.mjs'), [
    ...
    'export function apply(ctx, config = {}) {',
    "  writeFileSync(join(process.env.CF_HOME, 'witness'), String(config.generation ?? 'bundle-default'))",
    '}',
    '',
  ].join('\n'))
  writeFileSync(join(bundleDir, 'cordis.patch.yml'), [
...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  it('fully settles a custom profile, hot-reloads its patch layer with removal reverting, and disposes on a signal', async () => {
    ...
    try {
      ...
      // The home-level user layer ($CF_HOME/cordis.patch.yml) is live too
  ...
  describe('config dump', () => {
    ...
    it('prints the web profile bundle layers without a user layer', async () => {
      const { stdout, code, stderr } = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { CF_HOME: home })
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@origin-ai/cf-agent-loop'")
      expect(stdout).toContain('agents: []')
      expect(stdout).toContain('# == @origin-ai/cf-base')
      expect(stdout).toContain("name: '@origin-ai/cf-host-webserver'")
    }, 30_000)
    ...
    it('composes the profile user layer and a --patch overlay in order', async () => {
      ...
      const init = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { CF_HOME: home })
