...
const binScript = fileURLToPath(new URL('./fixtures/cf-badge/snapshot.ts', import.meta.url))
const configPath = fileURLToPath(new URL('./fixtures/cf-badge/cordis.yml', import.meta.url))
const defaultConfigPath = fileURLToPath(new URL('./fixtures/cf-badge/default.cordis.yml', import.meta.url))
const tsconfigPath = fileURLToPath(new URL('../../../tsconfig.json', import.meta.url))
const badgeAssetsPath = fileURLToPath(new URL('../../../packages/skill/skill-badge/assets/', import.meta.url))

describe('cf badge assembled snapshot', () => {
  it('advertises and loads the opt-in bundled skill through the shipped app', async () => {
    const disabled = await runLoaderSmoke({
      label: 'disabled cf badge skill snapshot',
      tempDirPrefix: 'headless-snapshot-cf-badge-disabled-',
      binScript,
    ...
    const enabled = await runLoaderSmoke({
      label: 'cf badge skill snapshot',
      tempDirPrefix: 'headless-snapshot-cf-badge-',
      binScript,
      libBinScript: binScript,
      configPath,
      tsconfigPath,
    ...
    expect(disabledSnapshot).toMatchInlineSnapshot(`
      {
        ...
        "result": {
          "content": [
            {
              "text": "Error: skill "cf-badge" is unknown or no longer available",
              "type": "text",
            },
          ],
          "error": {
            "message": "skill "cf-badge" is unknown or no longer available",
          },
          "isError": true,
        },
        "summary": null,
    ...
    expect(enabledSnapshot).toMatchInlineSnapshot(`
      ...
      A user may also invoke a skill directly; its <skill_content> block then appears in this conversation. Follow it, and do not call the \`skill\` tool again for that skill.
        ...
        "result": {
          "content": [
            {
              "text": "<skill_content name="cf-badge">
      <skill_resources>
      ...
      Add the official “powered by CodeFusion” badge without recreating or restyling it.

      ## Assets

      - Local PNG: [\`cf-badge.png\`](cf-badge.png), 726×120 source image; render at 121×20
      - Shields.io image URL: \`https://img.shields.io/badge/powered_by-codefusion-4D6BFE?style=flat-square\`
      - Project URL: \`https://github.com/origin-ai/code-fusion\`
      ...
      \`\`\`markdown
      [![](https://img.shields.io/badge/powered_by-codefusion-4D6BFE?style=flat-square)](https://github.com/origin-ai/code-fusion)
      \`\`\`

      If attribution should not be linked, use:

      \`\`\`markdown
      ![](https://img.shields.io/badge/powered_by-codefusion-4D6BFE?style=flat-square)
      \`\`\`
      ...
      - For Feishu and other systems that import remote images unreliably, upload \`cf-badge.png\` from this skill directory instead of generating another badge.
