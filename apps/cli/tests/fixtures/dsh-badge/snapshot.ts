...
if (overlayPath === undefined) throw new Error('xhe-badge snapshot requires an overlay path')
...
const ctx = await boot('xhe-badge-snapshot', rootConfigPath, [
  ...loadOverlayPatches('xhe-badge-snapshot', basePatchPath),
  ...loadOverlayPatches('xhe-badge-snapshot', overlayPath),
])

try {
  const agentId = SessionId('xhe-badge-snapshot')
  ...
  const agent: Agent = {
    ...
    followup: () => {},
    steer: () => {},
    inject: () => { throw new Error('xhe-badge snapshot must receive the catalog at the step boundary') },
    cancel: () => {},
  ...
  const catalog = decision.kind === 'enter'
    ...
    : undefined
  const summary = (await ctx.skills.list()).find(skill => skill.name === 'xhe-badge')
  const result = await ctx.tools.execute({
    callId: CallId('xhe-badge-snapshot'),
    name: 'skill',
