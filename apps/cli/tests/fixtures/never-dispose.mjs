...
export function apply(ctx) {
  ...
  process.stderr.write('xhe-test: never-dispose ready\n')
  ctx.effect(() => async () => {
    clearInterval(keepAlive)
    const armFile = process.env.XHE_TEST_SHUTDOWN_ARM_FILE
    if (armFile === undefined || !existsSync(armFile)) return
    process.stderr.write('xhe-test: never-dispose started\n')
    await new Promise(() => {})
  })
}
