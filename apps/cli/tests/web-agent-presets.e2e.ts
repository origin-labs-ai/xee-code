...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ cwd: proj })).map(skill => skill.name)).toEqual(['cf-badge'])
      ...
      expect(scoped).toContain('cf-badge')
      ...
      expect(JSON.stringify(loaded.content)).toContain('powered by CodeFusion')
    } finally {
      await handle.dispose()
    }
  })
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ scope: handle.agent })).map(skill => skill.name)).toContain('cf-badge')
