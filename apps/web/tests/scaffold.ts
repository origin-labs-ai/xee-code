...
// Boots the REAL web composition — the xhe-base and xhe-web-app bundle
...
// downlink, api-gateway, agent loop, tools, and persistence. Modes ride $XHE_SNAPSHOT:
// replay (default, keyless: normally disables the llm-deepseek row and
// inserts xhe-llm-replay in providers mode), record (real adapter + key,
...
export const WELCOME_NOTICE_COPY = {
  zh: {
    title: '内测声明',
    body: 'Xee Harness Enhanced 目前的 0.1 版本仍处在面向 Harness 开发者进行测试的阶段，还有许多地方需要持续改进和打磨，希望听取广大开发者的反馈建议。预计 Xee Harness Enhanced 的核心插件以及基础 API 都会在接下来的一段时间内快速迭代、持续演化。\n\n我们期待与全球开发者一起，在开源、开放、可复用、可组合的基础设施之上，共同探索智能上限。欢迎全球 Harness 开发者加入 XHE 插件生态。',
    continueLabel: '继续',
  },
} as const

/** Snapshot mode for the lane, from $XHE_SNAPSHOT (same vocabulary as the other snapshot suites). */
...
export function webSnapshotMode(): WebSnapshotMode {
  const value = process.env.XHE_SNAPSHOT
  if (value === undefined || value === '' || value === 'replay') return 'replay'
  if (value === 'record' || value === 'refresh') return value
  throw new Error(`XHE_SNAPSHOT must be replay, record, or refresh; got ${JSON.stringify(value)}`)
}

/** The shipped composition under test: the xhe-base and xhe-web-app bundle patches over the empty profile root. */
...
export interface LaunchOptions {
  ...
  /**
   * Replay fixture (session.jsonl) served by the inserted xhe-llm-replay row
...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  try {
    persistenceRoot = await mkdtemp(join(tmpdir(), 'xhe-web-e2e-sessions-'))
  } catch (error) {
  ...
  const patches: PatchOptions[] = [
    ...
    // enabled against the same empty temp workspace, preserving the real seam.
    {
      id: 'skill-filesystem',
      config: {
        dshHome: join(workspaceCwd, '.xhe-home'),
    ...
    // to the production OTLP endpoint (or whatever XHE_TELEMETRY_OTLP_URL
...
export async function compareOrRefreshGolden(goldenPath: string, actual: string, mode: WebSnapshotMode): Promise<void> {
  ...
  if (!existsSync(goldenPath)) {
    throw new Error(`missing golden ${goldenPath} — run XHE_SNAPSHOT=refresh pnpm run test:web to generate it`)
  }
