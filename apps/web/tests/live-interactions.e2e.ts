...
// composition and wire. The model adapter is xhe-llm-replay with override
...
import { deriveReplayScript, parseSessionLog } from '@origin-ai/xhe-llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  async function launch(
    ...
    if (buildOverride !== undefined) {
      ...
      sidecarDir = await mkdtemp(join(tmpdir(), 'xhe-web-e2e-sidecar-'))
