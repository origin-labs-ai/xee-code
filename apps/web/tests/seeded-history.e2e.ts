...
// fixture under the same record discipline as every other: XHE_SNAPSHOT=record drives the turn
...
import { deriveEventMessage, SessionId } from '@origin-ai/cf-session'
...
describe('web e2e: seeded history renders through cold resume', () => {
  ...
  it.skipIf(MODE === 'record')('reports full feedback correlation ids in an expandable two-line row', async () => {
    ...
    const previousDshHome = process.env.CF_HOME
    process.env.CF_HOME = scaffold.harnessHome
    try {
      const input = page.locator('textarea').first()
