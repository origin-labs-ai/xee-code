...
import { bootInjections, orderByModuleGraph } from '@origin-ai/cf-client-modules'
import type { ClientModuleLoaderTarget, WebBootEntry } from '@origin-ai/cf-client-modules/client'
...
const appBoot = await import(pathToFileURL(webBundleResolver.resolve('@origin-ai/cf-app-boot')).href) as unknown as BootComposition
...
export function mountAssembledApp(search = '?fixture'): void {
  ...
  for (const id of ['@origin-ai/cf-client-modules', '@origin-ai/cf-client-runtime']) {
