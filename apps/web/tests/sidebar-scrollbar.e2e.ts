...
interface ListMetrics {
  ...
  hoverRules: string[]
  /** `--xhe-scrollbar-thumb` resolved on the list, serialized as a colour. */
  token: string
  /** `--xhe-scrollbar-thumb-hover` resolved on the list, serialized the same way. */
  hoverToken: string
...
function measureList(page: Page): Promise<ListMetrics> {
  return page.evaluate(() => {
    ...
    return {
      ...
      hoverRules,
      token: resolve('--xhe-scrollbar-thumb'),
      hoverToken: resolve('--xhe-scrollbar-thumb-hover'),
...
interface PaletteMetrics {
  ...
  hovered: ListMetrics
  /** `--xhe-scrollbar-thumb` with the pointer parked outside the column. */
  quietThumb: string
}
...
function renderGeometry(light: PaletteMetrics, dark: PaletteMetrics): string {
  const palette = (name: string, { hovered: metrics, quietThumb }: PaletteMetrics): string[] => [
    `## ${name}`,
    '',
    `- --xhe-scrollbar-thumb, pointer outside the sidebar: ${quietThumb}`,
    ...
    `- --xhe-scrollbar-thumb, pointer over the list: ${metrics.token}`,
    `- --xhe-scrollbar-thumb-hover, pointer over the list: ${metrics.hoverToken}`,
...
/**
 * Resolve `--xhe-scrollbar-thumb` as the list sees it, without the rest of the
...
function resolveThumb(page: Page): Promise<string> {
  return page.evaluate(() => {
    ...
    if (list === null) throw new Error('sidebar session list not in the DOM')
    const probe = document.createElement('span')
    probe.style.color = 'var(--xhe-scrollbar-thumb)'
    list.append(probe)
    const value = getComputedStyle(probe).color
    probe.remove()
    return value
...
describe('web e2e: sidebar session list scrollbar (reserved gutter / themed thumb)', () => {
  ...
  it('renders the themed thumb through the WebKit path in both palettes', async () => {
    ...
    expect(light.hoverRules).toEqual(['var(--xhe-scrollbar-thumb-hover)'])
