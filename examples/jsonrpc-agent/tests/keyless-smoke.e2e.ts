...
describe('jsonrpc-agent keyless smoke', () => {
  it.each([
    ...
    const root = await mkdtemp(join(tmpdir(), 'xhe-jsonrpc-agent-smoke-'))
    ...
    const child = execa(process.execPath, [
      ...
      env: {
        ...
        XHE_CWD: root,
        XHE_SESSION_ROOT: join(root, '.sessions'),
        ...(envValue === undefined ? {} : { XHE_MAX_TOKENS_AS_SUCCESS: envValue }),
      },
      timeout: 35_000,
      killSignal: 'SIGKILL',
      reject: false,
  ...
  it('rejects an invalid max-token success env value', async () => {
    const { exitCode, stdout, stderr } = await execa(process.execPath, [
      ...
      env: {
        DEEPSEEK_API_KEY: 'keyless-smoke-no-call',
        XHE_MAX_TOKENS_AS_SUCCESS: 'sometimes',
      },
      stdin: 'ignore',
      timeout: 25_000,
      killSignal: 'SIGKILL',
    ...
    expect(stderr).toContain('failed to apply loader entry sdk-jsonrpc-server (@origin-ai/xhe-sdk-jsonrpc-server)')
