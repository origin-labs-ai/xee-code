...
const SESSION_PACKAGE = '@origin-ai/cf-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@origin-ai/cf-session/types'
 ...
 * top-level declaration (in `@origin-ai/cf-session`) and any declaration
 * merge inside a `declare module '@origin-ai/cf-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @origin-ai/cf-session/known-event-types',
    ' */',
    '',
    '/**',
