...
const SESSION_PACKAGE = '@origin-ai/xhe-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@origin-ai/xhe-session/types'
 ...
 * top-level declaration (in `@origin-ai/xhe-session`) and any declaration
 * merge inside a `declare module '@origin-ai/xhe-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @origin-ai/xhe-session/known-event-types',
    ' */',
    '',
    '/**',
