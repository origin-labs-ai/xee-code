...
describe('Vitest jsdom compatibility', () => {
  it('provides isolated browser storage instead of Node process storage', () => {
    ...
    localStorage.setItem('xhe-vitest-storage-probe', 'available')

    expect(localStorage.getItem('xhe-vitest-storage-probe')).toBe('available')
    localStorage.removeItem('xhe-vitest-storage-probe')
  })
})
