...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@origin-ai/cf-client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@origin-ai/cf-a/subpath'",
      "declare module '@origin-ai/cf-client-ui-slots' {}",
      "const load = () => import('@origin-ai/cf-b')",
...
describe('package modes', () => {
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@origin-ai/cf-client-runtime/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('dependency sections', () => {
  it('accepts dynamic peer plus dev relationships, static dev inputs, and private dependencies', () => {
    ...
    const runtime = pkg('runtime', {
      inject: ['@origin-ai/cf-client-feature'],
      sourceUses: {
        '@origin-ai/cf-agent': ['packages/client/runtime/src/index.ts'],
        '@origin-ai/cf-client-ui-slots': ['packages/client/runtime/src/client/slots.ts'],
        react: ['packages/client/runtime/src/client/view.tsx'],
      },
      dependencies: { immer: '^10.1.1' },
      peerDependencies: {
        [CORDIS]: 'workspace:^',
        '@origin-ai/cf-agent': 'workspace:^',
        '@origin-ai/cf-client-feature': 'workspace:^',
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@origin-ai/cf-agent': 'workspace:^',
        '@origin-ai/cf-client-feature': 'workspace:^',
        '@origin-ai/cf-client-ui-slots': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('rejects internal dependencies, static peers, and mismatched peer development ranges', () => {
    ...
    const subject = pkg('feature', {
      sourceUses: {
        '@origin-ai/cf-agent': ['packages/client/feature/src/index.ts'],
        [slots.name]: ['packages/client/feature/src/view.tsx'],
      },
      dependencies: { '@origin-ai/cf-agent': 'workspace:^' },
  ...
  it('allows npm dependency cycles', () => {
    const a = pkg('a', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@origin-ai/cf-client-b': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@origin-ai/cf-client-b': 'workspace:^' },
    })
    const b = pkg('b', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@origin-ai/cf-client-a': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@origin-ai/cf-client-a': 'workspace:^' },
    })
...
describe('module requests', () => {
  it('accepts a dynamic row supplier and its client subpath', () => {
    const ui = declaration('ui', { external: ['@origin-ai/cf-client-slots/client'] })
  ...
  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@origin-ai/cf-client-ui', '@origin-ai/cf-missing', '@origin-ai/cf-missing'],
      inject: ['', '@origin-ai/cf-a', '@origin-ai/cf-a'],
    })
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = declaration('a', {
      external: ['@origin-ai/cf-client-b'],
      inject: ['@origin-ai/cf-client-b'],
    })
    const b = declaration('b', {
      external: ['@origin-ai/cf-client-a'],
      inject: ['@origin-ai/cf-client-a'],
    })
...
describe('manifest declarations', () => {
  ...
  it('fixes unambiguous dependency sections and declaration entries', () => {
    ...
    roots.push(root)
    const subject = pkg('feature', {
      external: ['', 'react', '@origin-ai/cf-client-feature', '@origin-ai/cf-missing'],
      inject: ['', '@origin-ai/cf-agent', '@origin-ai/cf-agent'],
      sourceUses: {
        '@origin-ai/cf-agent': ['packages/client/feature/src/index.ts'],
        '@origin-ai/cf-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@origin-ai/cf-agent': 'workspace:*',
    ...
    expect(fixed.dsh.client).toMatchObject({
      external: ['@origin-ai/cf-missing'],
      inject: ['@origin-ai/cf-agent'],
    })
