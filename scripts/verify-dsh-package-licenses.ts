/**
 * Enforce the MIT license declaration for repository-owned XHE npm packages.
 * @module scripts/verify-xhe-package-licenses
 */
...
const XHE_PACKAGE_NAME = /^@deepseek-ai\/dsh(?:-|$)/

/** Result of checking every XHE package reachable through the root workspace list. */
export interface DshPackageLicenseReport {
  /** Number of XHE package manifests checked. */
  packageCount: number
...
function readManifest(root: string, file: string): Record<string, unknown> {
  ...
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    throw new Error(`verify-xhe-package-licenses: ${file} must contain a JSON object.`)
  }
...
function workspaceManifestPaths(root: string): string[] {
  ...
  if (!isStringArray(workspaces)) {
    throw new Error('verify-xhe-package-licenses: package.json workspaces must be a string array.')
  }
...
/**
 * Check every XHE npm package declared by the repository workspace.
...
export function inspectDshPackageLicenses(root: string): DshPackageLicenseReport {
  ...
  for (const file of workspaceManifestPaths(root)) {
    ...
    if (typeof name !== 'string' || !XHE_PACKAGE_NAME.test(name)) continue
...
if (process.argv[1] && import.meta.filename === resolve(process.argv[1])) {
  ...
  if (report.failures.length > 0) {
    process.stderr.write('verify-xhe-package-licenses: non-MIT XHE package declarations found:\n')
    for (const failure of report.failures) process.stderr.write(`  ${failure}\n`)
    process.exitCode = 1
  } else {
    process.stdout.write(
      `verify-xhe-package-licenses: ${String(report.packageCount)} XHE package(s) checked; all declare MIT.\n`,
    )
  }
}
