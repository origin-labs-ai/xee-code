...
function workspaceFixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'xhe-project-reference-faces-'))
  roots.push(root)
  ...
  writeJson(join(shared, 'package.json'), { name: '@origin-ai/xhe-shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@origin-ai/xhe-split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@origin-ai/xhe-host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@origin-ai/xhe-client-consumer' })
