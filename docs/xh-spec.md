...
1.1 This document normatively defines X Harness (XH), a plugin-based, multi-model agent harness that coordinates heterogeneous model instances through a GOD Runtime, a discussion bus, a tiered memory fabric, a dynamic skill framework, and gated verification.

1.2 XH is provider-agnostic. Provider, credential, model, and runtime instance are distinct concepts (Section 5). No implementation may assume a one-credential-one-model binding.

1.3 Existing single-agent execution remains available. XH enhanced orchestration is opt-in and governed by explicit configuration (Section 4).
...
3.1 XH follows a layered architecture: User, Entry, GOD Runtime, Model Mesh, Skills, Discussion, Verification, Memory, and Storage.
...
```mermaid
flowchart TB
    USER["USER"] --> XH["XH Harness"]
    XH --> NORMAL["Standard Single-Agent Path"]
    XH --> ENHANCED["XH Enhanced Orchestration"]
    NORMAL --> DIRECT["Direct Execution"]
    ENHANCED --> CONFIG["XH Configuration"]
...
4.1 XH MUST support two entry paths from the same harness invocation: standard single-agent execution and enhanced orchestration. The enhanced path MUST be explicit in configuration; absence of enhanced configuration MUST NOT change standard behavior.
...
Cost is not linear in instance count. XH SHOULD implement cheap exploration, selective escalation, and expensive verification. Example: N fast instances explore, K promising hypotheses escalate, one strong verifier decides.
...
13.1 When the run is production-near (post-Gauntlet or post-build convergence), XH MUST run a four-agent parallel sweep:
...
flowchart LR
    subgraph UserSide["User Side"]
        U["User Request"] --> XH["XH Harness"]
        XH --> GOD["GOD Runtime"]
        XH --> SIMPLE["Single-Agent Path"]
    end
...
*End of X Harness normative specification.*
