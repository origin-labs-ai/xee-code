...
This is also why service replacement works in config: unload the `xhe-bash-local` entry, mount a different `shell` provider, and every plugin injecting `'shell'` cleanly restarts against the new implementation.
