...
```json
{
  "name": "xhe-hello-plugin",
  "version": "0.1.0",
  "type": "module",
  "main": "index.js",
  "files": ["index.js", "cordis.patch.yml"],
  "xhe": { "bundle": { "patch": "./cordis.patch.yml" } }
}
```
...
The first use initializes the profile (with `@origin-ai/xhe-base` as its first bundle), pnpm links the checkout, and `dsh` appends the bundle to `dsh.profile.bundles` because the package declares `dsh.bundle`:

```json
{
  "name": "xhe-profile-demo",
  "private": true,
  "dependencies": {
    "xhe-hello-plugin": "link:/path/to/hello-plugin"
  },
  "xhe": {
    "profile": {
      "bundles": [
...
```sh
dsh --profile demo --dump-config   # shows a "# == xhe-hello-plugin" layer
dsh --profile demo
```

`dsh plugin --profile demo remove xhe-hello-plugin` removes both the dependency and the layer.

## The loading order

The effective configuration composes over an empty root by applying, in order:

1. Each bundle patch named in the profile's `dsh.profile.bundles` list, in list order — `@origin-ai/xhe-base` first, then each installed bundle in the order it was added.
...
- Your patch can override rows from earlier layers by `id` — the same way [the `xhe-web-app` bundle](../../../../packages/bundle/web-app/cordis.patch.yml) overrides `xhe-base` rows — but must restate every key the row needs, not just the changed one.
...
In-box bundle names always resolve from the dsh installation itself; pnpm manages only out-of-tree packages, so your bundle can rely on `@origin-ai/xhe-base` being present and current.
...
```yaml
- id: hello-startup
  name: 'xhe-hello-plugin/startup'
```

The plugin exports `inject = ['cmdlineArgs']`, calls `parseCmdline` from [`@origin-ai/xhe-cmdline`](../../../../packages/boot/cmdline/README.md) with its own commander program, and provides its app-owned service from the program's action. The launcher hands every plugin the same immutable arguments after launcher flags, so app-specific flags need no launcher change and multiple plugins may parse the snapshot. The Loader row needs no launcher marker or special kind.
...
- **The author** ships a `prepare` script — pnpm runs it after a git install — that builds the published entry points from source, self-contained: it must not assume dev-only context such as a sibling monorepo checkout. [turtle-ui](https://github.com/xhe/turtle-ui) is a working example: its `prepare` runs a dedicated tsdown config that transpiles `src/` without project references or type checking.
