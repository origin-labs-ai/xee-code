 ...
 * @module @origin-ai/cf-agent-spine-demo
 */
...
import SessionTitleService, { type Config as SessionTitleConfig } from '@origin-ai/cf-session-title'
import SystemPrompt, { type Config as SystemPromptConfig } from '@origin-ai/cf-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@origin-ai/cf-tools'
import SkillRegistry, { type Config as SkillRegistryConfig } from '@origin-ai/cf-skill'
import * as SkillFileSystem from '@origin-ai/cf-skill-filesystem'
import AgentRegistry from '@origin-ai/cf-agent'
import GoalService, { type Config as GoalDomainConfig } from '@origin-ai/cf-goal'
import * as goalSession from '@origin-ai/cf-goal-round-driver'
import * as toolGoal from '@origin-ai/cf-tool-goal'
import LocalJobRegistry, { type Config as JobsConfig } from '@origin-ai/cf-jobs-local'
import InvariantRegistry, { type Config as InvariantConfig } from '@origin-ai/cf-invariants'
...
import * as bashEnv from '@origin-ai/cf-shell-env'
import * as workspaceContext from '@origin-ai/cf-agent-instructions'
import * as toolSkill from '@origin-ai/cf-tool-skill'
import * as toolJobs from '@origin-ai/cf-tool-jobs'
import AgentLoop, { type Config as AgentLoopConfig } from '@origin-ai/cf-agent-loop'
import * as llmRetry from '@origin-ai/cf-llm-retry'
 ...
 * `cfHome` to bash environment and local skill discovery, `sessionTitle` to
...
export interface Config {
  ...
  tools?: ToolsConfig
  /** CodeFusion home directory shared by shell context and local skill discovery. */
  cfHome?: string
...
export const Config = z.intersect([
  ...
  }) as unknown as z<Pick<Config, 'tools' | 'cfHome' | 'sessionTitle' | 'skills' | 'workspaceContext' | 'toolBash' | 'jobs' | 'toolJobs' | 'invariants' | 'goals'>>,
...
export function pickSpineConfig(config: Omit<Config, 'agents'>): Omit<Config, 'agents'> {
  return {
    ...
    ...config.cfHome !== undefined ? { cfHome: config.cfHome } : {},
...
export function apply(ctx: Context, config: Config): void {
  const nestedCfHome = config.skills?.filesystem?.cfHome
  if (config.cfHome !== undefined && nestedCfHome !== undefined
    && resolveCfHome(config.cfHome) !== resolveCfHome(nestedCfHome)) {
    throw new Error('agent-spine-demo: cfHome and skills.filesystem.cfHome must resolve to the same directory')
  }
  const cfHome = resolveCfHome(config.cfHome ?? nestedCfHome)
  ...
  if (skillsEnabled) {
    ...
    ctx.plugin(SkillFileSystem, Object.assign({}, config.skills?.filesystem, { cfHome }))
  }
