 ...
 * `xhe-acp-demo [--config path]`, defaulting to `./cordis.yml`. Shared env
 * loading, Loader guards, snapshot config selection, and settled-tree boot live
 * in xhe-app-boot. Replay skips `.env` and selects sibling
...
/* v8 ignore start -- thin self-executing composition over the unit-tested
   xhe-app-boot helpers; exercised end-to-end by the snapshot suite and the
   built-bin smoke */
installFailLoud(NAME)
const snapshotMode = process.env['XHE_SNAPSHOT']
