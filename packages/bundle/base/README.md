...
- **The Windows temp grant is a private per-session subdirectory** — `workspace-write` confines writes to the workspace plus the session's own temp subdirectory (`<temp>\xhe-<hash>`, TMP/TEMP rewritten for confined children); `read-only` grants nothing. See `@origin-ai/xhe-sandbox-windows-acl`.
