...
const KEY = credentialRef('XHE_CRED_TEST')
const OTHER = credentialRef('XHE_CRED_OTHER')
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'xhe-credentials-local-'))
...
describe('layering and reads', () => {
  ...
  it('lets a non-empty process environment win read-only over the file', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'version: 1\nrefs:\n  XHE_CRED_TEST: from-file\n')
    const ctx = await boot({ path, watch: false })
    vi.stubEnv('XHE_CRED_TEST', 'from-env')
  ...
  it('treats an empty environment value as absent, falling through to the file', async () => {
    const dir = await tempDir()
    const path = join(dir, '.credentials.yaml')
    await writeCredentials(path, 'version: 1\nrefs:\n  XHE_CRED_TEST: stored\n')
    const ctx = await boot({ path, watch: false })
    vi.stubEnv('XHE_CRED_TEST', '')
...
describe('layer ladder', () => {
  ...
  async function bootLayered(
    ...
    const ctx = new Context()
    ctx.provide(XHE_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot(layers))
  ...
  it('lets the stored value beat the user .env, so a UI write takes effect immediately', async () => {
    ...
    const ctx = await bootLayered(path, [
      ...
      { source: 'user-env', path: '/home/.dsh/.env', values: { XHE_CRED_TEST: 'older-user-env' } },
    ])
    expect(await ctx.credentials.resolve(KEY)).toEqual({ value: 'stored', source: 'file' })
  ...
  it('serves the user .env only when nothing is stored', async () => {
    ...
    const ctx = await bootLayered(join(dir, '.credentials.yaml'), [
      { source: 'process', values: {} },
      { source: 'user-env', path: '/home/.dsh/.env', values: { XHE_CRED_TEST: 'from-user-env' } },
    ])
  ...
  it('serves the invoking project .env over the user one, but never over the store', async () => {
    ...
    const layers = [
      ...
      { source: 'project-env' as const, path: '/work/.env', values: { XHE_CRED_TEST: 'from-project' } },
      { source: 'user-env' as const, path: '/home/.dsh/.env', values: { XHE_CRED_TEST: 'from-user' } },
    ]
  ...
  it('lets only the inherited environment shadow the store, read-only', async () => {
    ...
    await writeCredentials(path, 'version: 1\nrefs:\n  XHE_CRED_TEST: stored\n')
    const ctx = await bootLayered(path, [
      { source: 'process', values: { XHE_CRED_TEST: 'from-shell' } },
      { source: 'user-env', path: '/home/.dsh/.env', values: { XHE_CRED_TEST: 'from-user-env' } },
    ])
...
describe('document validation', () => {
  ...
  it.each([
    ...
    ['the flat layout with a non-string value', 'XHE_CRED_TEST: [nope]\n', /nest the existing 1 entry under/],
    ['the flat layout with several entries and a non-string value', 'XHE_CRED_TEST: a\nXHE_CRED_OTHER: [b]\n',
      /nest the existing 2 entries under/],
    ['the flat layout with an empty value', 'XHE_CRED_TEST: ""\n', /pre-release flat layout/],
    ...
    ['malformed yaml', 'XHE_CRED_TEST: "unterminated\n', /invalid document/],
  ...
  it('never puts a credential value in a diagnostic', async () => {
    ...
    await writeCredentials(path, `XHE_CRED_TEST: "${secret}\n`)
    let failure: unknown
    try {
...
describe('document writes', () => {
  ...
  it('rejects empty values and writes the environment would shadow', async () => {
    ...
    vi.stubEnv('XHE_CRED_TEST', 'shadowing')
  ...
  it('fails a write loud when the on-disk document became invalid', async () => {
    ...
    await writeCredentials(path, 'XHE_CRED_TEST: "unterminated\n')
