...
import type { SubagentListEntry } from '@origin-ai/xhe-subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'xhe-tool-list-agents-'))
  roots.push(root)
...
describe('xhe-tool-subagent-control/list-agents', () => {
