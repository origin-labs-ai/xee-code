...
import type { SubprocessHandle } from '@origin-ai/xhe-subprocess'
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)(
  ...
  () => {
    it('returns one unique nonce through the production provider and real SDK/CLI', async () => {
      ...
      const root = mkdtempSync(join(tmpdir(), 'xhe-claude-deepseek-e2e-'))
      roots.push(root)
      ...
      const nonce = `XHE_CLAUDE_DEEPSEEK_${randomUUID()}`
      const parent = {
