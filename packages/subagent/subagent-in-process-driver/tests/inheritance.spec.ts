...
import SandboxedFileSystem from '@origin-ai/cf-fs-sandbox'
...
beforeEach(async () => {
  workspace = await realpath(await mkdtemp(join(tmpdir(), 'xhe-inherit-')))
})
  ...
  it('records the parent sandbox override and the approval pin before publishing a spawn child', async () => {
    ...
    try {
      ...
      const contextText = runtimeContext.data.content
        ...
        .map(block => block.text)
        .join('\n')
      expect(contextText).toContain('Current XHE file policy: read-only')
