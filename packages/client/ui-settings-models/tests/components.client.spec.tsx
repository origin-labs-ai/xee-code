...
import type { RpcResponse, SettingsNamespaceView } from '@origin-ai/xhe-api-remotes/client'
import {
...
const DEEPSEEK_TARGET = { provider: 'deepseek-official', displayName: 'CodeFusion' }
  ...
  const face = {
    llm: {
      providers: vi.fn(() => Promise.resolve(ok({
        providers: [
          { provider: 'deepseek-official', displayName: 'CodeFusion', settingsNs: 'llm-deepseek', settingsPath: [], active: true },
...
describe('ModelsSection', () => {
  ...
  it('renders the unkeyed whole-section provider as an open setup card in the first-run posture', async () => {
    ...
    expect(screen.getByText('CodeFusion')).toBeTruthy()
  ...
  it('leaves the unkeyed provider a plain row once another provider is usable', async () => {
    ...
    expect(missing.closest('li')?.textContent).toContain('CodeFusion')
  ...
  it('uses one stable provider identity in action copy', () => {
    const target = { provider: 'deepseek-official', displayName: 'CodeFusion' }
    expect(providerTargetLabel(target)).toBe('CodeFusion (deepseek-official)')
    expect(providerCopy(en.deleteTitle, target)).toBe('Delete CodeFusion (deepseek-official)?')
  ...
  it('stores a typed key write-only from the setup card without touching settings', async () => {
    ...
    expect((await screen.findByRole('status')).textContent).toBe(
      providerCopy(en.savedProvider, { provider: 'deepseek-official', displayName: 'CodeFusion' }),
    )
  ...
  it('collapses the setup card on cancel without disturbing another open card', async () => {
    ...
    expect(screen.getAllByRole('img', { name: en.credentialMissing })
      .some(dot => dot.closest('li')?.textContent?.includes('CodeFusion') === true)).toBe(true)
  ...
  it('loads on first render of an idle controller', async () => {
    ...
    render(<ModelsSection
      ...
      t={t}
    />)
    await screen.findByText('CodeFusion')
  })
