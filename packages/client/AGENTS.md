...
Packages here are named with the directory prefix: `@origin-ai/cf-client-<name>`.
...
2. **A dynamic package declares internal dynamic relationships as peer plus dev.** Production source imports, re-exports, module augmentations, and type-only references to an `@origin-ai/cf-*` package count, as does a package named by `dsh.client.inject`. A test-only internal dependency stays dev-only.
