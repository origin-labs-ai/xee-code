...
import { createScope } from '@origin-ai/xhe-client-runtime/client'
...
import type { CommandContribution, SelectOption } from '@origin-ai/xhe-client-ui-commands/client'
...
describe('ui-model-selection dual entry', () => {
  ...
  it('popup options mark the host current active with the provider group in the detail', async () => {
    ...
    expect(options[0]).toMatchObject({ active: true, detail: 'CodeFusion' })
