...
import SessionTitleService, { foldSessionTitle } from '@origin-ai/xhe-session-title'
...
describe('session title persistence round trips', () => {
  it('round-trips through a remounted JSONL backend', async () => {
    const root = await mkdtemp(join(tmpdir(), 'xhe-title-jsonl-'))
    roots.push(root)
  ...
  it('round-trips through a remounted SQLite backend', async () => {
    const root = await mkdtemp(join(tmpdir(), 'xhe-title-sqlite-'))
    roots.push(root)
