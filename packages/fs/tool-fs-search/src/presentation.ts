 ...
 * model-facing render consumes ({@link module:@origin-ai/cf-tool-fs-search/search-core}
 ...
 * final output budget (`xhe-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @origin-ai/cf-tool-fs-search/presentation
 */
 ...
 * ({@link module:@origin-ai/cf-tool-fs-search/grep} `formatGrepMatches`), so
