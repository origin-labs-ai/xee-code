 ...
 * `xhe-fs-observation-policy`, so it is not exercised here.
 */
...
import { FsVersion } from '@origin-ai/cf-fs'
import type { FsTarget } from '@origin-ai/cf-fs'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'xhe-fs-'))
  ctx = new Context()
...
describe('resolve', () => {
  it('resolves a relative path against opts.cwd, not config.cwd', async () => {
    ...
    const other = await mkdtemp(join(tmpdir(), 'xhe-fs-other-'))
    try {
