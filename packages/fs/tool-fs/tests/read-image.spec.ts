...
import { CallId, LlmAdapter, LlmRuntime } from '@origin-ai/cf-llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, Message, StreamChunk } from '@origin-ai/cf-llm'
import SystemPrompt from '@origin-ai/cf-system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@origin-ai/cf-tools'
import type { Config as ToolConfig } from '@origin-ai/cf-tools'
import LocalFileSystem from '@origin-ai/cf-fs-local'
import * as FsPolicy from '@origin-ai/cf-fs-observation-policy'
import LocalAttachmentStore from '@origin-ai/cf-attachment-local'
import { AttachmentError, AttachmentId, AttachmentStore } from '@origin-ai/cf-attachment'
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { cfHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { cfHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { cfHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
