...
Every foreground and background model pwsh call receives a freshly collected trusted `XHE_*` environment through the shared [`xhe-shell-env`](../shell-env/) registry: `CF_HOME` (the absolute Harness home), `XHE_SHELL=1`, the agent's `XHE_SESSION_ID`, and `XHE_SESSION_JSONL` when the active persistence backend locates one. Plugins contributing `XHE_*` facts to `ctx.shellEnv` apply to pwsh calls exactly as they do to bash calls. The snapshot passes through the dedicated `ShellExecRequest.cfEnv` channel; `process.env` is never modified. The description teaches the generic `$env:XHE_*` convention rather than naming persistence-specific variables.
