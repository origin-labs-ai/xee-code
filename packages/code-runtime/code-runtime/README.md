...
| `language` | Readonly descriptor: the source language `run` expects. `'typescript'` and `'python'` are the well-known values — those `xhe-tools` presents; only `'typescript'` has a published backend. Informational, not gating — a consumer that generates language-specific presentation switches on it and fails loud on a language it cannot present. |
...
Indirectly, through Code Mode in `xhe-tools`, which exposes `run_code` and returns program logs, values, or failures as retained tool-result tokens.
