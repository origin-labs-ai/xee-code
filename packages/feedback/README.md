...
A command feedback remark is log-only: it never enters model context or derived history. When mounted, [`xhe-session-telemetry-otel`](../session/session-telemetry-otel) observes `feedback/record` to release a pending telemetry prefix or warn that disabled telemetry leaves the feedback local; capture itself remains independent of that policy.
