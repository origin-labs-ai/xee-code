...
import { cfHomePath, resolveCfHome } from '@origin-ai/cf-home-paths'
...
export {
  ...
  type CfBundleManifest,
  type CfManifestSection,
  type CfProfileManifest,
  type Profile,
  type ProfileLayer,
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The CodeFusion implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend CodeFusion itself.`,
  })
}
