 ...
 * `cf --profile` launcher family.
 *
 * A profile is a directory under `$CF_HOME/profiles/<name>` holding a
 ...
 * @module @origin-ai/cf-app-boot/profile
 */
...
/** The bundle half of the `cf` manifest section: what a bundle package exports. */
export interface CfBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The profile half of the `cf` manifest section: what a profile directory composes. */
export interface CfProfileManifest {
  /** Ordered bundle layer list (package names). */
  bundles?: string[]
}

/**
 * The profile-launcher slice of the `cf`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface CfManifestSection {
  /** Bundle metadata consumed by the profile launcher. */
  bundle?: CfBundleManifest
  /** Profile metadata consumed by the profile launcher. */
  profile?: CfProfileManifest
}
...
export interface ProfileManifest {
  ...
  cf?: CfManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `cf.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `cf.profile.bundles` order. */
  layers: ProfileLayer[]
 ...
 * @param name - the profile name (`cf --profile <name>`).
 * @param home - the Harness home; defaults to {@link resolveCfHome}.
 * @returns the absolute profile directory (which may not exist yet).
 */
export function resolveProfileDir(name: string, home: string = resolveCfHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    // The launcher-maintained flat module fallback lives at this sibling path.
    || name === 'node_modules') {
    throw new Error(`cf: invalid profile name ${JSON.stringify(name)}`)
  }
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['@origin-ai/cf-base', '@origin-ai/cf-web-app'],
  headless: ['@origin-ai/cf-base', '@origin-ai/cf-headless'],
}

/** Installation-owned bundle tuples normalized to the shipped template. */
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@origin-ai/cf-base', '@origin-ai/cf-web-app', '@origin-ai/cf-headless'],
}

/** The bundle list a `cf plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@origin-ai/cf-base']

const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this cf profile, applied after every bundle layer:
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`cf: ${link} exists and is not a symlink; remove it so cf can manage the installation fallback`)
    }
...
/**
 * Maintain the flat module fallback `$CF_HOME/profiles/node_modules`: one
 * symlink per package in the cf app's resolvable dependency CLOSURE (BFS
 ...
 * @param installAnchor - absolute path of the cf app's package.json.
 * @param home - the Harness home; defaults to {@link resolveCfHome}.
 */
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveCfHome()): void {
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.cf?.profile?.bundles
  ...
  const normalized: ProfileManifest = {
    ...
    cf: {
      ...manifest.cf,
      profile: { ...manifest.cf?.profile, bundles: [...current] },
    },
  }
 ...
 * `@origin-ai/cf-base` (and every other in-box bundle) always comes from
 * the same installation as the running cf, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `cf.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the cf app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the cf installation or ${profileDir}; `
    + `run 'cf plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}

/**
 * Load a profile: resolve every `cf.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `cf.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the cf app's package.json (first resolution anchor).
 * @param home - the Harness home; defaults to {@link resolveCfHome}.
...
export function loadProfile(
  binName: string, name: string, installAnchor: string, home: string = resolveCfHome(),
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'cf plugin --profile ${name} add <package>'`,
      )
    }
    initProfile(dir, template)
  }
  const manifest = normalizeShippedProfile(name, dir, readProfileManifest(binName, dir))
  // A hand-written profile manifest may omit the cf section entirely.
  const bundles = manifest.cf?.profile?.bundles ?? []
  const layers = bundles.map((packageName): ProfileLayer => {
    const packageDir = resolveBundleDir(binName, packageName, installAnchor, dir)
    const bundleManifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as ProfileManifest
    const declared = bundleManifest.cf?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no cf.bundle in its package.json`)
    }
