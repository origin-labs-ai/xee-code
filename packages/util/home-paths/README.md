...
Shared filesystem path helpers for Xee Harness Enhanced user data.

## XHE home

`resolveDshHome()` resolves the single-root Xee Harness Enhanced home. Precedence, highest first: an explicit configured path, `$XHE_HOME`, then `~/.dsh`. The harness keeps all user data under one root.
...
`dshHomeDisplay()` names an active root symbolically for user-facing paths: `~/.dsh` for the default home, `$XHE_HOME` for any configured home. It never leaks an absolute machine path.

`XHE_HOME_DIR_NAME` owns the default user-data directory name: `.dsh`.

`defaultDshHome()` returns the default Xee Harness Enhanced home by joining the operating-system home directory with `.dsh`, using Node's platform path rules.
