 ...
 * execution pipeline.
 * @module @origin-ai/xhe-tools
 */

import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@origin-ai/xhe-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@origin-ai/xhe-scope'
import type { CallId, ContentBlock, ToolSchema } from '@origin-ai/xhe-llm'
import { assertNever, deepFreeze, HarnessError } from '@origin-ai/xhe-llm'
import type { Agent } from '@origin-ai/xhe-agent'
import { snapshotJsonValue } from '@origin-ai/xhe-session'
import type { JsonValue, UserMessage } from '@origin-ai/xhe-session'
import type { ToolProviderResult } from '@origin-ai/xhe-system-prompt'
import type { CodeRuntime } from '@origin-ai/xhe-code-runtime'
 ...
 * instead of deriving them: the seam's `xhe-code-runtime` README pair, its
...
export type { JsonValue } from '@origin-ai/xhe-session'
...
// lives in its own UI-facing module; re-export it so `@origin-ai/xhe-tools`
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@origin-ai/xhe-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@origin-ai/xhe-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@origin-ai/xhe-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@origin-ai/xhe-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@origin-ai/xhe-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `xhe-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`xhe-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`xhe-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @origin-ai/xhe-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`xhe-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `xhe-agent-tool-presentation` exists for, and reading the deployment default would
