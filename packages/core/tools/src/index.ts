 ...
 * execution pipeline.
 * @module @origin-ai/cf-tools
 */

import { Context, Service } from '@deepseek-ai/cordis'
import z from '@deepseek-ai/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@origin-ai/cf-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@origin-ai/cf-scope'
import type { CallId, ContentBlock, ToolSchema } from '@origin-ai/cf-llm'
import { assertNever, deepFreeze, HarnessError } from '@origin-ai/cf-llm'
import type { Agent } from '@origin-ai/cf-agent'
import { snapshotJsonValue } from '@origin-ai/cf-session'
import type { JsonValue, UserMessage } from '@origin-ai/cf-session'
import type { ToolProviderResult } from '@origin-ai/cf-system-prompt'
import type { CodeRuntime } from '@origin-ai/cf-code-runtime'
...
export type { JsonValue } from '@origin-ai/cf-session'
...
// lives in its own UI-facing module; re-export it so `@origin-ai/cf-tools`
...
declare module '@deepseek-ai/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@origin-ai/cf-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@origin-ai/cf-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@origin-ai/cf-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@origin-ai/cf-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@origin-ai/cf-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`xhe-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @origin-ai/cf-code-runtime-worker-thread) or set tools mode to "native"`)
    }
