 ...
 * @module @origin-ai/xhe-tools/src/code-mode
 */

import { CallId, createUserMessage, HarnessError } from '@origin-ai/xhe-llm'
import type { ContentBlock } from '@origin-ai/xhe-llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@origin-ai/xhe-code-runtime'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`xhe-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
