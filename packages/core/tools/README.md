...
- **`timeoutMs` on a definition is declarative only** — the registry never enforces deadlines; enforcement requires the `@origin-ai/cf-tool-call-timeout-policy` wrapper.
