...
The user-global file is always `$XHE_HOME/AGENTS.md` with no local overlay; both candidate lists only control project scopes. `$XHE_HOME` defaults to `~/.dsh`, and configured `~`, `~/...`, and Windows-style `~\...` prefixes are expanded against the operating-system home directory. A non-positive or non-finite render budget disables both baseline and dynamic loading; configured `maxSourceBytes` must be a positive integer.
