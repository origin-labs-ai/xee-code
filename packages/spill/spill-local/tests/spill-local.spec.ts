...
import type { SaveTextSpill } from '@origin-ai/xhe-spill'
import LocalSpillStore, { encodeSegment, privateRoot, saveTextFile, sessionDir } from '@origin-ai/xhe-spill-local'

let root: string

beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'xhe-spill-test-'))
})
afterEach(() => {
