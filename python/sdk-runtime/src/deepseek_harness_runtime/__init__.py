...
- **exe (production)**: single-file Node executables named
  ``cf-jsonrpc-agent-pkg-<platform>-<arch>`` (platform in {linux, macos}, arch in
...
- **node (dev-only)**: the full deploy closure under ``runtime/node/``
  ...
  runtime/node/node_modules/@origin-ai/cf-sdk-jsonrpc-demo/lib/packaged-bin.js`` on a
...
def bundled_runtime_path() -> Path:
    ...
    path = bundled_package_dir() / "runtime" / f"cf-jsonrpc-agent-pkg-{tag}"
    if not path.is_file():
        raise FileNotFoundError(
...
def _current_platform_tag() -> str:
    ...
    if plat is None or arch is None:
        raise FileNotFoundError(
            "no bundled cf-jsonrpc-agent executable exists for this platform "
