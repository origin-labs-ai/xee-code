...
class RuntimeBuildHook(BuildHookInterface):
    ...
    def initialize(self, version: str, build_data: dict[str, object]) -> None:
        ...
        platform_tag = os.environ.get("CF_RUNTIME_PLATFORM_TAG") or _host_platform_tag()
        ...
        if len(matches) != 1:
            ...
            raise RuntimeError(
                f"unsupported CF_RUNTIME_PLATFORM_TAG {platform_tag!r}; expected one of {supported}"
            )
        expected_executable = matches[0][1]
        runtime_dir = Path(self.root) / "src" / "deepseek_harness_runtime" / "runtime"
        runtime_files = sorted(runtime_dir.glob("cf-jsonrpc-agent-pkg-*") if runtime_dir.is_dir() else [])
